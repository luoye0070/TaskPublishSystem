...
KindEditor.plugin('filemanager', function(K) {
	...
	function bindTitle(el, data) {
		if (data.is_dir) {
			el.attr('title', data.origin_name);
		} else {
			el.attr('title', makeFileTitle(data.origin_name, data.filesize, data.datetime));
		}
	}
	self.plugin.filemanagerDialog = function(options) {
		...
		function bindEvent(el, result, data, createFunc) {
			...
			if (data.is_dir) {
				...
				el.click(function(e) {
					clickFn.call(this, fileUrl, data.origin_name);
				});
			} else {
				el.click(function(e) {
					clickFn.call(this, fileUrl, data.origin_name);
		...
		function createList(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				var iconUrl = imgPath + (data.is_dir ? 'folder-16.gif' : 'file-16.gif'),
					...
					cell0 = K(row[0].insertCell(0)).addClass('ke-cell ke-name').append(img).append(document.createTextNode(' ' + data.origin_name));
				if (!data.is_dir || data.has_file) {
					row.css('cursor', 'pointer');
					cell0.attr('title', data.origin_name);
		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				var img = K('<img src="' + iconUrl + '" width="80" height="80" alt="' + data.origin_name + '" />');
				if (!data.is_dir || data.has_file) {
				...
				photoDiv.append(img);
				div.append('<div class="ke-name" title="' + data.origin_name + '">' + data.origin_name + '</div>');
			}
		}
