...
class TpsResourceFileController {
    def tpsResourceFileService;
    ...
    //资源文件列表
    def list() {
        def reInfo = tpsResourceFileService.search(params);
//        reInfo<<[errors:errors,warnings:"warnings"]
        log.info("reInfo-->"+reInfo);
        render(view: "/front/resourceFile/list", model: reInfo);
    }
    //预览flash文件
    def previewFlash(){
        def errors=null;
        ...
        render(view: "/front/resourceFile/previewFlash", model: [errors:errors,urlStr:urlStr,isSwf:isSwf,isMp3:isMp3]);
    }
    //删除资源文件
    def delete() {
        log.info("params->" + params);
        def reInfo = tpsResourceFileService.deleteFile();
    ...
    def upload() {
        ...
        if (request.method == "POST") {
            def reInfo = tpsResourceFileService.uploadFile();
            log.info("reInfo-->" + reInfo);
            if (reInfo.recode == ReCode.OK) {
        ...
        render(view: "/front/resourceFile/upload", model: [isSaveOk:isSaveOk,success: success, errors: errors]);
    }
    def diyUploadPage(){
...
//        log.info("fileLimitSizeStr-->" + fileLimitSizeStr);
        ...
        render(view: "/front/resourceFile/diyUpload", model: [fileLimitSize:fileLimitSizeStr]);
    }
    //diyUPload上传文件
    def diyUpload(){
        render (tpsResourceFileService.uploadFile() as JSON)
    }
    //在编辑器中上传文件
    def uploadInEditor(){
        def errors = "";
...
//        out.close();
    ...
    def manageInEditor(){
        ...
        def resourceList=null;//ResourceFileInfo.findAllByType(fileType.code);
        Member member=springSecurityService.currentUser;
        if(member){
            resourceList=TpsResourceFileInfo.findAllByTypeAndMember(fileType.code,member);
        }
        //遍历目录取的文件信息
